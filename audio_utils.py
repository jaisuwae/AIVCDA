...
def speak(text):
    ...
    if use_cloud:
        if not is_connected():
            ...
            engine.say("Internet not detected. Using offline voice.")
            engine.runAndWait()
        else:
            try:
...
def speak_stream(text):
    """Speak partial text chunks as they arrive from the local LLM stream."""
    if not text:
        return
    # Use offline voice output for streaming chunks so Silvia starts speaking early.
    try:
        engine.say(text)
        engine.runAndWait()
    except Exception as e:
...
def listen(wav_path):
    """Hybrid listener: Uses Google STT if connected, else falls back to Vosk."""
    config = load_config()
    use_google = config.get("USE_GOOGLE_STT", True)
    
    if use_google and is_connected():
        try:
            ...
            print(f"Google STT unexpected error: {e}")

    # Local Vosk Fallback
    print("Processing with Local Vosk STT...")
